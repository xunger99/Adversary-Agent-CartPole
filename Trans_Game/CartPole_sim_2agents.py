# Xun's test code
#
# Set-ups:  Model: simple, 8 inputs, 2cp env with ks=20, reward: win+500, lose-100. Fit two offensive agents together.
#
...
# Define a s agent (a very small and simple network)
...
#model1 =  agent_NN(full_obs_shape, ctr_shape)
model2 = agent_NN2(full_obs_shape, ctr_shape)
model1.summary()
...
def Xun_fit2(agent1, agent2, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
    ...
    did_abort = False
     ...
     # for plotting learning curve. xun
    step_vs_reward =[0]
    ag1_episode_reward = [0]    
    ag2_episode_reward = [0]    
    ...
    try:
        while agent1.step < nb_steps:
            ...
            if done:
                ...
                # Trace all episode rewards
                #pdb.set_trace()
                step_vs_reward +=[agent1.step]
                ag1_episode_reward += [episode_reward1]
                ag2_episode_reward += [episode_reward2]
    ...
    return step_vs_reward, ag1_episode_reward, ag2_episode_reward   #history1, history2
    

    

#agent1.load_weights('1CP_SARSA_4_weights.h5f')
#agent2.load_weights('1CP_SARSA_8_weights.h5f')
...
step_vs_reward, ag1_episode_reward, ag2_episode_reward = Xun_fit2(agent1, agent2, env, nb_steps = 100001, verbose = 1)
env.env.close()
#pdb.set_trace()

agent1.save_weights('CL3Ag3_double_SARSA_agent1.h5f', overwrite=True)
agent2.save_weights('CL3Ag3_double_SARSA_agent2.h5f', overwrite=True)
#env.close()
    


# Plot the learning curves (in terms of rewards)
fig = plt.figure(figsize=(10,5))
ax1 = plt.subplot(121)
ax1.plot(step_vs_reward, ag1_episode_reward, 'b-')      # Plot the bang-bang force
ax1.set_xlabel(r"Steps", fontsize=15,color='k')
ax1.set_ylabel(r"Rewards", fontsize=15,color='k')
ax1.set_title('Agent 1')

ax2 = plt.subplot(122)
ax2.plot(step_vs_reward, ag2_episode_reward, 'r-')      # Plot the bang-bang force
ax2.set_xlabel(r"Steps", fontsize=15,color='k')
