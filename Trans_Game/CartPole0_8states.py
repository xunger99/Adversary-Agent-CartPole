...
model1 = agent_NN2(full_obs_shape, ctr_shape)
model1.summary()
...
# Use the agent's fit.
#agent1.fit(env, nb_steps = 50001, visualize = False, verbose = 1)
...
def Xun_fit(agent, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
    ...
    try:
        while agent.step < nb_steps:
            ...
            #obs=observation        #4states
            obs=np.append(observation,observation)   #8states
            action = agent.forward(obs)      # for each agent... xun
            ...
            for _ in range(action_repetition):
                ...
                obs = np.append(observation, observation)       # 8 states
                reward += r                                     
                if done:
                    break
...
Xun_fit(agent1, env, nb_steps = 50001, visualize = False, verbose = 1)
env.env.close()

# Next, save the model, load again and test in a new env
#agent1.save_weights('1CP_SARSA_4_weights.h5f', overwrite=True)
agent1.save_weights('1CP_SARSA_8_weights.h5f', overwrite=True)

#pdb.set_trace()
# load the weights
#agent1.load_weights('1CP_SARSA_4_weights.h5f')
agent1.load_weights('1CP_SARSA_8_weights.h5f')
